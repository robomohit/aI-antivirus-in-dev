...
def scan_file_content(file_path: str) -> Dict[str, List[str]]:
    ...
    """
    try:
        # Import signature scanning functions
        from signatures import scan_file_content as sig_scan
        return sig_scan(file_path)
    except ImportError:
        # Fallback if signatures module not available
        return {}
...
def check_behavior_flags(file_path: str) -> Tuple[int, List[str]]:
    ...
    """
    try:
        # Import behavior checking functions
        from signatures import check_behavior_flags as behavior_check
        return behavior_check(file_path)
    except ImportError:
        ...
        try:
            ...
            suspicious_patterns = ['stealer', 'keylogger', 'backdoor', 'trojan', 'virus']
            ...
            if extension in ['.exe', '.bat', '.cmd', '.com', '.scr', '.pif']:
                behavior_score += 1
...
def get_signature_match_summary(threats_found: Dict[str, List[str]]) -> str:
    ...
    """
    try:
        from signatures import get_signature_match_summary as sig_summary
        return sig_summary(threats_found)
    except ImportError:
