#!/usr/bin/env python3
"""
Create Balanced Realistic Dataset
Generate a more balanced dataset to prevent overfitting
"""
...
class BalancedDatasetCreator:
    ...
    def create_realistic_malware_features(self, count=300):
        """Create realistic malware features with more variation."""
        print(f"{Fore.CYAN}🔄 Creating {count} realistic malware samples...")
        
        malware_features = []
        
        # More diverse malware patterns with overlap
        malware_patterns = [
            # Ransomware - high entropy, low printable
            {"entropy_range": (7.0, 8.0), "printable_range": (0.1, 0.3), "size_range": (50000, 2000000)},
            # Trojan - moderate entropy, moderate printable
            {"entropy_range": (6.0, 7.5), "printable_range": (0.2, 0.5), "size_range": (100000, 500000)},
            # Cryptominer - very high entropy
            {"entropy_range": (7.5, 8.0), "printable_range": (0.1, 0.25), "size_range": (200000, 1000000)},
            # Keylogger - lower entropy, more printable
            {"entropy_range": (5.5, 6.5), "printable_range": (0.3, 0.6), "size_range": (50000, 300000)},
            # Backdoor - moderate entropy
            {"entropy_range": (6.5, 7.5), "printable_range": (0.2, 0.4), "size_range": (80000, 400000)}
        ]
        ...
        for i in range(count):
            ...
            # Generate realistic features with more variation
            ...
            # More realistic feature ranges
            strings_count = np.random.randint(1, 100)  # More variation
            avg_string_length = np.random.uniform(2.0, 10.0)  # More variation
            histogram_regularity = np.random.uniform(0.4, 1.0)  # More overlap
            entropy_consistency = np.random.uniform(0.6, 1.2)  # More overlap
    ...
    def create_realistic_benign_features(self, count=700):
        """Create realistic benign features with more variation."""
        print(f"{Fore.CYAN}🔄 Creating {count} realistic benign samples...")
        
        benign_features = []
        
        # More diverse benign patterns with overlap
        benign_patterns = [
            # Text files - low entropy, high printable
            {"entropy_range": (3.0, 4.5), "printable_range": (0.8, 0.95), "size_range": (100, 100000)},
            # Configuration files - moderate entropy
            {"entropy_range": (4.0, 5.5), "printable_range": (0.7, 0.9), "size_range": (500, 50000)},
            # Legitimate executables - moderate entropy, moderate printable
            {"entropy_range": (5.0, 6.5), "printable_range": (0.4, 0.7), "size_range": (10000, 50000000)},
            # System files - moderate entropy
            {"entropy_range": (5.5, 6.8), "printable_range": (0.3, 0.6), "size_range": (5000, 1000000)},
            # Document files - moderate entropy
            {"entropy_range": (4.5, 6.0), "printable_range": (0.6, 0.8), "size_range": (1000, 100000)},
            # Large legitimate files - moderate entropy
            {"entropy_range": (5.0, 6.5), "printable_range": (0.5, 0.7), "size_range": (100000, 10000000)}
        ]
        ...
        for i in range(count):
            ...
            # More realistic feature ranges with overlap
            strings_count = np.random.randint(10, 2000)  # More variation
            avg_string_length = np.random.uniform(5.0, 25.0)  # More variation
            histogram_regularity = np.random.uniform(0.1, 0.6)  # More overlap
            entropy_consistency = np.random.uniform(0.1, 0.8)  # More overlap
    ...
    def save_dataset(self, malware_features, benign_features):
        ...
        filename = f"balanced_realistic_dataset_{timestamp}.csv"
    ...
    def run_creation_process(self, malware_count=300, benign_count=700):
        """Run the complete dataset creation process."""
        print(f"{Fore.CYAN}🛡️  BALANCED DATASET CREATION")
        ...
        print(f"{Fore.CYAN}📊 Balanced malware patterns created")
        print(f"{Fore.CYAN}📊 Balanced benign patterns created")
...
def main():
    """Main function."""
    print(f"{Fore.CYAN}🛡️  Starting Balanced Dataset Creation...")
    
    creator = BalancedDatasetCreator()
    creator.run_creation_process(malware_count=300, benign_count=700)
