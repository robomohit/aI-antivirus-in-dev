#!/usr/bin/env python3
"""
Create Enhanced Malware Dataset for AI Training (Windows Optimized)
...
def create_malware_dataset():
    ...
    safe_print("Creating Enhanced Malware Dataset v5.X")
    safe_print("=" * 50)
    ...
    safe_print(f"Generating {malware_samples} malware samples...")
    ...
    safe_print(f"Generating {safe_samples} safe samples...")
    ...
    safe_print("Dataset created successfully!")
    safe_print(f"Total samples: {len(df)}")
    safe_print(f"Malware samples: {len(df[df['is_malicious'] == 1])}")
    safe_print(f"Safe samples: {len(df[df['is_malicious'] == 0])}")
    safe_print(f"Saved to: malware_dataset.csv")
    
    # Print feature summary
    safe_print("\nFeature Summary:")
    safe_print(f"   - File size range: {df['file_size_kb'].min():.1f} - {df['file_size_kb'].max():.1f} KB")
    safe_print(f"   - Entropy range: {df['entropy_score'].min():.2f} - {df['entropy_score'].max():.2f}")
    safe_print(f"   - Behavior score range: {df['behavior_score'].min()} - {df['behavior_score'].max()}")
    safe_print(f"   - Extensions: {df['extension'].nunique()} unique types")
    safe_print(f"   - Categories: {df['file_category'].nunique()} categories")
