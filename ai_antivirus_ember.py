#!/usr/bin/env python3
"""
AI Antivirus with EMBER Model
Uses the proven EMBER model for reliable malware detection
"""
...
class EMBERAIAntivirus:
    def __init__(self):
        self.models_dir = "ember_real_models"
        self.quarantine_dir = "quarantine"
        self.log_file = "antivirus_ember.log"
    ...
    def _load_model(self):
        """Load the EMBER model."""
        print(f"{Fore.CYAN}🔄 Loading EMBER model...")
        
        try:
            # Find latest model files
            model_files = list(Path(self.models_dir).glob("ember_real_model_*.pkl"))
            metadata_files = list(Path(self.models_dir).glob("ember_real_metadata_*.pkl"))
            
            if not model_files or not metadata_files:
                print(f"{Fore.RED}❌ No EMBER model found!")
                print(f"{Fore.YELLOW}💡 EMBER model should be in ember_real_models/")
                return
            ...
            print(f"{Fore.GREEN}✅ EMBER model loaded: {latest_model.name}")
            print(f"{Fore.GREEN}✅ Metadata loaded: {latest_metadata.name}")
            print(f"{Fore.CYAN}📊 Features: {len(self.metadata.get('feature_cols', []))}")
            
        except Exception as e:
            print(f"{Fore.RED}❌ Error loading EMBER model: {e}")
    ...
    def extract_ember_features(self, file_path):
        ...
        try:
            ...
            # Calculate EMBER-style features
            file_size = len(data)
    ...
    def analyze_file(self, file_path):
        """Analyze a single file using the EMBER model."""
        ...
        try:
            ...
            # Extract features
            features = self.extract_ember_features(file_path)
            if features is None:
                return None
            ...
            # Determine threat level with EMBER thresholds
            if prediction == 1:
                if probability > 0.8:
                    threat_level = "HIGH"
    ...
    def _generate_detection_reason(self, features, probability, prediction):
        ...
        if prediction == 1:  # Malware detected
            ...
            return " | ".join(reasons) if reasons else "EMBER model detected malware"
        else:
    ...
    def scan_directory(self, directory_path, scan_mode="quick"):
        """Scan a directory for malware using EMBER model."""
        directory = Path(directory_path)
        if not directory.exists():
            print(f"{Fore.RED}❌ Directory not found: {directory_path}")
            return
        
        print(f"{Fore.CYAN}🛡️  EMBER AI ANTIVIRUS")
        ...
        # Print final results
        print(f"\n{Fore.CYAN}{'='*60}")
        print(f"{Fore.CYAN}📊 EMBER SCAN RESULTS")
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="EMBER AI Antivirus")
